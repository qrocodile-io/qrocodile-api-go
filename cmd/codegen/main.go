...
func run(check bool, specURL string) error {
	...
	enumValues, err := generateEnumValues(generated)
	if err != nil {
		return fmt.Errorf("generating enum value lists: %w", err)
	}

	outPath := filepath.Join(repoRoot, "internal", "openapi", "types.gen.go")
	enumValuesPath := filepath.Join(repoRoot, "internal", "openapi", "enumvalues.gen.go")
	...
	if check {
		for _, f := range []struct {
			...
			if err != nil {
				return fmt.Errorf("reading committed %s: %w", f.path, err)
			}
			if !bytes.Equal(existing, f.content) {
				return fmt.Errorf("%s is out of date with %s — run `go run ./cmd/codegen` and commit the result", f.path, specURL)
			}
		}
	...
	if err := os.WriteFile(enumValuesPath, enumValues, 0o644); err != nil {
		return err
	}
	fmt.Println("codegen: wrote", outPath, "and", enumValuesPath)
	return nil
}

// generateEnumValues parses generated (types.gen.go's contents) and emits a second Go file
// declaring, for every string-enum type oapi-codegen produced (every type with at least one
// associated `const` of that type), a `<Type>Values = []<Type>{...}` slice listing every valid
// value in the order oapi-codegen already emitted them (alphabetical by generated constant
// name). oapi-codegen has no built-in equivalent — it emits individual constants and a Valid()
// method per enum type, but nothing that enumerates them — and hand-writing these lists would
// be exactly the kind of hand-copied, driftable enum this project has avoided everywhere else.
// Parsing the generated source (rather than re-deriving the value lists from the OpenAPI schema
// directly) means every emitted value is guaranteed to reference a constant oapi-codegen itself
// generated, so there's no way for the two files to disagree.
func generateEnumValues(generated []byte) ([]byte, error) {
	fset := token.NewFileSet()
	file, err := parser.ParseFile(fset, "types.gen.go", generated, 0)
	if err != nil {
	...
	for _, decl := range file.Decls {
		...
		for _, spec := range genDecl.Specs {
			...
			constsByType[typeName] = append(constsByType[typeName], valueSpec.Names[0].Name)
		}
	}

	typeNames := make([]string, 0, len(constsByType))
	for typeName := range constsByType {
		typeNames = append(typeNames, typeName)
	}
	sort.Strings(typeNames)

	var b bytes.Buffer
	b.WriteString("// Code generated by cmd/codegen from types.gen.go's own constants. DO NOT EDIT.\n\n")
	b.WriteString("package openapi\n\n")
	for _, typeName := range typeNames {
		constNames := constsByType[typeName]
		fmt.Fprintf(&b, "// %sValues lists every valid %s, in the order oapi-codegen generated them.\n", typeName, typeName)
		fmt.Fprintf(&b, "var %sValues = []%s{\n", typeName, typeName)
	...
	if err != nil {
		return nil, fmt.Errorf("formatting generated enum values: %w", err)
	}
